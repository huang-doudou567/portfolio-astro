...
export const projects: Project[] = [
  {
    ...
    coreFeatures: [
      ...
      { title: '9 视图全栈 SPA', desc: 'Chat（DeepSeek SSE 流式对话）、Dashboard（评分卡片+情绪分布饼图）、Journal（流水账编辑器）、Recall Timeline（情绪窗格筛选+搜索）、Patterns（盲区/正向模式）、Story（叙事层）、Decision Sim（三路径推演）、Onboarding（5 步引导）、User Guide（9 FAQ）。React 19 + TypeScript 6 + Vite 8 + TailwindCSS 4。' },
    ],
  ...
  },
  {
    ...
    coreFeatures: [
      { title: '六大场景独立 Agent', desc: '每个经营场景对应一个独立 Agent，各有专属 System Prompt 和工具组合：清仓 Agent（库存+进价→三档降价方案）、选品 Agent（品类+价格→TOP 潜力品）、竞品对标 Agent（全维度拆解→差异化方案）、促销 Agent（预算→最优组合）、差评归因 Agent（文本→根因分类）、月进货 Agent（销量+库存→预算约束采购清单）。' },
      { title: '单品全维度分析（6 维）', desc: '输入商品链接，输出：市场定位（低价/主流/高端）、价格竞争力（★ 评分可视化）、机会与风险（空白带识别+竞争拥挤度）、竞品对比表、语料洞察（品类高频关键词分布）、选品推荐（综合性价比评分 TOP N）。' },
      { title: '每条建议可追溯到源头（核心差异化）', desc: '每条选品建议附带三层证据链：数据证据（具体数字和来源文件）、置信度标注（样本量够不够？数据新鲜度如何？）、局限性声明（盲区是什么）。这是和 ChatGPT"凭感觉给建议"最本质的区别。' },
      { title: '5+1 面板数据看板', desc: '首页总览（8500）、策略面板——CR3/CR5 集中度热力图+价格带空白点识别（8501）、单品 6 维分析报告（8502）、Prompt 在线调优——保存即生效（8503）、决策追踪——三个月自动回看（8504）、Web Chat 流式聊天（5173）。' },
    ],
  ...
  },
  {
    ...
    coreFeatures: [
      ...
      { title: '意图识别 —— 不只是关键词匹配', desc: '五种意图类型各有独立搜索策略：政策查询（自动纠偏税种）、申报指导（全文搜索+官方解读优先）、合规风险（关联法条+风险点标注）、优惠资格判定（条件匹配检索）、发票处理（流程类+罚则类双重检索）。' },
      { title: '4 Tab 法规弹窗 —— 一个法规的四层信息', desc: '点击任意结果展开四标签页：📖 法规原文（全文+高亮+章节分类）→ 🔍 官方解读（答记者问/立法说明）→ 🤖 AI 通俗版（适用主体/核心要点/注意事项）→ 🌐 相关网页（行业分析/第三方视角）。四个 Tab 的数据来自 5 个不同数据源，用户感知到的只是一个弹窗。' },
      { title: '智能引导面板 —— 解决"不知道查什么"', desc: '4 步向导：选择身份 → 选择税种（12 个卡片）→ 选择意图 → 补充条件。多数企业主不会说"增值税留抵退税"，他们说"我进项比销项多怎么办"——引导面板把这个翻译成法律术语。' },
      { title: '8 条硬约束 —— 信任体系', desc: '每条回答必须包含：查询时间戳 + 数据来源标注 + 时效性徽章 + NPC 法规 ID + 免责声明。8 条禁止行为（不替用户做决定、不编造案例、失效法规标记……）是写在最高优先级约束里的硬规则，不是建议。' },
    ],
