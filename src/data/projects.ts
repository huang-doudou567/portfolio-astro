...
export const projects: Project[] = [
  {
    ...
    architecture: '三层递进式架构：底层案例蒸馏引擎（构建结构化经验智库，150+ 维度特征框架，采集-蒸馏-特征化-决策四层管线）→ 中层六层递进式认知架构（事件归档→情绪识别→检索召回→特征提取→模式发现→路径推演，六层解耦独立迭代）→ 顶层安全护栏（10 条负面约束清单，全本地零服务端存储）。\n\n从"概率生成"到"循证决策"的完整产品逻辑。',
    results: [
